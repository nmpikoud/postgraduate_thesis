...
int main(){
	...
	#pragma omp parallel\
	shared(initial_conditions,h,moic,a,filename_format)\
	private(j,i,y0,x0,k1,k2,k3,k4,y,y1,filename,x1)\
	firstprivate(p1,p2,xi,xf)\
	default(none)
	{
	
		int myid=omp_get_thread_num();
		int nthreads=omp_get_num_threads();
		int istart=myid*moic/nthreads+1;
		int iend=(myid+1)*moic/nthreads;
		if(myid==nthreads-1){iend=moic;}
...
void der(double z[],double x,double y[]){
	...
	*/
	...
	double eyilon=0.01;// Magnitude of perturbation
	z[0]=2*A*y[2]+4*B*y[2]*(y[0]*y[0]+y[1]*y[1]+y[2]*y[2]+y[3]*y[3])+2*C*y[2]+E*y[3]-K*y[3]-2*R*y[2]-eyilon*1.48882E-7*y[0];
	z[1]=2*A*y[3]+4*B*y[3]*(y[0]*y[0]+y[1]*y[1]+y[2]*y[2]+y[3]*y[3])+2*D*y[3]+E*y[2]-K*y[2]-2*S*y[3]-eyilon*2.20505E-6*y[1];
	z[2]=-2*A*y[0]-4*B*y[0]*(y[0]*y[0]+y[1]*y[1]+y[2]*y[2]+y[3]*y[3])-2*C*y[0]-E*y[1]-F-K*y[1]-2*R*y[0]-eyilon*1.48882E-7*y[2];
	z[3]=-2*A*y[1]-4*B*y[1]*(y[0]*y[0]+y[1]*y[1]+y[2]*y[2]+y[3]*y[3])-2*D*y[1]-E*y[0]-I-K*y[0]-2*S*y[1]-eyilon*2.20505E-6*y[3];
}
